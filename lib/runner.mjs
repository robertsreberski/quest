...
import { getAdapter } from "./workers.mjs";
...
const HELP = {
  ...
  usage: [
    ...
    "              [--max-iterations N] [--max-cost USD] [--continue-session]",
    "              [--notify '<cmd>'] [--dry-run] [--json]",
...
function resolveOptions(front, config, flags) {
  ...
  return { worker, model, effort, maxIterations, maxCost, maxTokens };
}
    ...
    if (initial.status === "complete") {
      ...
      for (;;) {
        ...
        const ctx = {
          ...
          checkpointsBefore,
          spawn: (inv) => spawnCapture(resolveExecutable(inv.cmd, env), inv.args, { cwd: workerCwd, env: { ...env, QUEST_DIR: storeDir, ...inv.env } }),
...
export async function run(argv, io = {}) {
  ...
  try {
    ...
    if (config.backend === "github") {
      errOut("quest-run: the github backend is not yet available in this build");
      errOut("  hint: use backend \"local\" for now");
      return 6;
    }
