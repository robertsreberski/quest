...
function exitCodeFor(err) {
  ...
  if (err instanceof BackendError) return 6;
  return 1;
}
...
function getStore(cwd, env) {
  ...
  if (config.backend === "github") {
    throw new BackendError("the github backend ships in a later quest of this build and is not available yet", { hint: "use backend \"local\" for now" });
  }
  return config;
}
...
async function dispatch(argv, ctx) {
  ...
  if (!command) {
    ...
    const config = loadConfig(storeDir, env);
    const all = config.backend === "local" ? local.listQuests(storeDir) : [];
    const counts = {};
    ...
    out(renderStatusOverview({ counts, ready: config.backend === "local" ? local.readyQuests(storeDir) : [], activeRuns: active, backend: config.backend }));
    return 0;
  }
...
const HANDLERS = {
  async init(args, { out, cwd, env }) {
    ...
    if (backend === "github") {
      throw new BackendError("the github backend ships in a later quest of this build and is not available yet", { hint: "run `quest init` (local) for now" });
    }
    const storeDir = local.initLocalStore(cwd);
    const config = {
      backend,
      ...(p.values.repo ? { github: { repo: p.values.repo } } : {}),
      defaults: { worker: "claude", claude: { model: "opus", effort: "xhigh" }, codex: { model: "gpt-5-codex", reasoning_effort: "medium" }, max_iterations: 8, priority: "p2" },
      notify: { command: "" },
    };
  ...
  async create(args, { out, cwd, env }) {
    ...
    const created = local.createQuest(config.storeDir, config.defaults, {
      title: v.title,
      priority: v.priority,
      worker: v.worker,
      model: v.model,
  ...
  async list(args, { out, cwd, env }) {
    ...
    const config = getStore(cwd, env);
    let quests = p.values.ready ? local.readyQuests(config.storeDir) : local.listQuests(config.storeDir);
  ...
  async show(args, { out, cwd, env }) {
    ...
    const config = getStore(cwd, env);
    const q = local.loadQuest(config.storeDir, id);
  ...
  async start(args, { out, cwd, env }) {
    ...
    const config = getStore(cwd, env);
    const q = local.startQuest(config.storeDir, id);
  ...
  async checkpoint(args, { out, cwd, env }) {
    ...
    const config = getStore(cwd, env);
    const existing = local.loadQuest(config.storeDir, id);
    const q = local.appendCheckpoint(config.storeDir, id, {
  ...
  async cancel(args, { out, cwd, env }) {
    ...
    const config = getStore(cwd, env);
    local.cancelQuest(config.storeDir, id, p.values.reason);
  ...
  async edit(args, { out, cwd, env }) {
    ...
    const config = getStore(cwd, env);
    local.editQuest(config.storeDir, id, {
  ...
  async lint(args, { out, errOut, cwd, env }) {
    ...
    let results;
    if (p.values.all) {
      results = local.lintAll(config.storeDir);
    } else {
