...
export function installAgents({ scope = "project", dryRun = false, force = false, cwd = process.cwd(), env = process.env } = {}) {
  ...
  for (const name of AGENTS) {
    ...
    if (existsSync(dest) && sameFile(dest, text)) {
      actions.push({ name, path: dest, action: "unchanged" });
      continue;
    }
    if (existsSync(dest) && !force) {
      conflicts.push({ name, path: dest });
      actions.push({ name, path: dest, action: "conflict" });
      continue;
    }
    actions.push({ name, path: dest, action: existsSync(dest) ? "replace" : "create" });
    if (!dryRun) {
  ...
  return { ok: conflicts.length === 0, scope, target: dir, dry_run: dryRun, actions, conflicts };
}

export function doctor({ cwd = process.cwd(), env = process.env } = {}) {
  const checks = [];
  const versions = versionInfo();
  const manifestVersion = versions.codex;

  checks.push(check(
    "version-sync",
    versions.package === versions.codex && versions.package === versions.claude,
    `package=${versions.package}, codex=${versions.codex}, claude=${versions.claude}`,
    { versions },
  ));
  ...
  if (codexVersion.status === 0) {
    ...
    const hookWarning = /hook.*(parse|warning|error|failed)|parse warning/i.test(debug.stderr || "");
    checks.push(check(
      "hook-parser",
  ...
  checks.push(check(
    "native-agents",
    missing.length === 0,
    missing.length ? `missing: ${missing.join(", ")}` : `found: ${Object.values(found).join(", ")}`,
    { found, missing },
  ));
