...
function providerRecommendation(provider, checks) {
  ...
  if (provider === "codex") {
    ...
    if (!setupReady) {
      return {
        ...
        available: false,
        command: `${questBin()} codex doctor --fix`,
    ...
    if (!bad("multi-agent-feature") && !bad("goals-feature")) {
      return {
        ...
        available: true,
        command: `${questBin()} codex work ${commandId} --dry-run`,
    ...
    if (!bad("goals-feature")) {
      return {
        ...
        available: true,
        command: `${questRunBin()} ${commandId} --worker codex --codex-goal-mode require`,
    ...
    return {
      ...
      available: true,
      command: `${questRunBin()} ${commandId} --worker codex --codex-goal-mode auto`,
  ...
  if (!setupReady) {
    return {
      ...
      available: false,
      command: `${questBin()} claude doctor --fix`,
  ...
  return {
    ...
    available: true,
    command: `${questBin()} claude work ${commandId} --dry-run`,
