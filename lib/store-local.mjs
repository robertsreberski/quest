...
import { ContractError, assertTransition, lintRecord, makeCheckpoint, nowIso, parseRecord, recordFilename, serializeRecord, CHECKPOINT_MARKER } from "./contract.mjs";
...
function appendUnderCheckpoints(body, block) {
  const hasSection = body.split("\n").some((l) => l.trim() === "## Checkpoints");
  const trimmed = body.replace(/\n+$/, "");
  return hasSection ? `${trimmed}\n\n${block}\n` : `${trimmed}\n\n## Checkpoints\n\n${block}\n`;
}

function appendToSection(body, section, line, { createAfter } = {}) {
